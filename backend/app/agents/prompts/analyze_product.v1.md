...
7. 如果 visual_input_count=0，visual_observations 必须为空。
8. inferred_category 可以根据商品名称、卖点、平台、目标人群和图片可见事实归纳为简短表达类别。
9. readiness_score 表示当前资料进入创意规划的完整度和清晰度；资料越具体，分数越高。
10. 不输出解释性正文，只输出符合给定 JSON schema 的对象。
